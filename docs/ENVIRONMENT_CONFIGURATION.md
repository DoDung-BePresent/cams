...
2. **Chỉnh sửa `.env` với giá trị local của bạn**
   ...
   ConnectionStrings__DefaultConnection=Host=localhost;Port=5432;Database=logaicams_db;...
   ```
...
```hcl
resource "aws_ecs_task_definition" "logaicams_api" {
  family                   = "logaicams-api"
  ...
  container_definitions = jsonencode([{
    name  = "logaicams-api"
    image = "your-ecr-repo/logaicams-api:latest"
...
resource "aws_secretsmanager_secret" "db_connection" {
  name = "logaicams/db/connection-string"
}

resource "aws_secretsmanager_secret_version" "db_connection" {
  secret_id = aws_secretsmanager_secret.db_connection.id
  secret_string = jsonencode({
    connection_string = "Host=${aws_rds_cluster.main.endpoint};Port=5432;Database=logaicams_db;Username=${var.db_username};Password=${var.db_password};"
  })
}

resource "aws_secretsmanager_secret" "jwt_key" {
  name = "logaicams/jwt/secret-key"
}
...
```hcl
resource "aws_ecs_service" "logaicams_api" {
  name            = "logaicams-api"
  cluster         = aws_ecs_cluster.main.id
  task_definition = aws_ecs_task_definition.logaicams_api.arn
  desired_count   = 2
  launch_type     = "FARGATE"
...
resource "aws_iam_role_policy" "ecs_secrets" {
  name = "logaicams-ecs-secrets-policy"
...
services:
  logaicams-api:
    build:
      context: .
      dockerfile: ./src/LogAICAMS.API/Dockerfile
    environment:
...
```bash
docker run -d \
  -e ConnectionStrings__DefaultConnection="Host=db;Port=5432;Database=logaicams_db;..." \
  -e Jwt__Key="your-secret-key" \
  -e ASPNETCORE_ENVIRONMENT=Production \
  logaicams-api:latest
```
