...
export const PairDeviceModal = ({
  ...
  const [generateErrorMessage, setGenerateErrorMessage] = useState<
    string | null
  >(null);
  const autoGeneratedForOpenRef = useRef(false);
  ...
  const handleGeneratePairCode = useCallback(async () => {
    ...
    try {
      ...
      setGenerateErrorMessage(
        response.data.message || 'Unable to generate pair code.',
      );
    } catch (error) {
      const fallback = 'Unable to generate pair code.';
      const axiosMessage = (
        error as { response?: { data?: { message?: string } } }
      )?.response?.data?.message;
      setGenerateErrorMessage(axiosMessage || fallback);
    }
  }, [spaceId, generatePairCode]);
  ...
  const handleAfterOpenChange = useCallback(
    async (isOpen: boolean) => {
      if (!isOpen || !spaceId) {
        autoGeneratedForOpenRef.current = false;
        return;
      }
      ...
      const deviceInfoResult = await refetchPairDeviceInfo();
      if (autoGeneratedForOpenRef.current) {
        return;
      }

      if (!deviceInfoResult.data?.deviceSessionId) {
        autoGeneratedForOpenRef.current = true;
        await handleGeneratePairCode();
      }
    },
    [spaceId, refetchPairDeviceInfo, handleGeneratePairCode],
  );
