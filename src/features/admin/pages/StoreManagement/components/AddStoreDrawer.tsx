 ...
 * Features
 */
import type { CreateStorePayload } from '@/features/admin/types/storeTypes';
...
type AddStoreDrawerProps = {
  open: boolean;
  onClose: () => void;
  onSuccess: (store: CreateStorePayload) => void;
};
...
export const AddStoreDrawer = ({
  ...
  const [form] = Form.useForm<CreateStorePayload>();
  const [loading, setLoading] = useState(false);

  const handleSubmit = async (values: CreateStorePayload) => {
    try {
      setLoading(true);
